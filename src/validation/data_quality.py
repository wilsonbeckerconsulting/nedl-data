...
def record_stat(report: DQReport, category: str, metric: str, value: str, description: str = "") -> None:
...
def validate_data_quality(
    ...
    if len(dim_property) > 0:
        dq_check(report, 'REQUIRED_FIELD', 'dim_property.tax_assessor_id NOT NULL',
                 sum(1 for p in dim_property if p.get('tax_assessor_id')), len(dim_property))
        dq_check(report, 'REQUIRED_FIELD', 'dim_property.valid_from NOT NULL',
                 sum(1 for p in dim_property if p.get('valid_from')), len(dim_property))
    
    # dim_entity required fields
    if len(dim_entity) > 0:
        dq_check(report, 'REQUIRED_FIELD', 'dim_entity.canonical_entity_id NOT NULL',
                 sum(1 for e in dim_entity if e.get('canonical_entity_id')), len(dim_entity))
        dq_check(report, 'REQUIRED_FIELD', 'dim_entity.canonical_entity_name NOT NULL',
    ...
    if len(fact_transaction) > 0:
        dq_check(report, 'REQUIRED_FIELD', 'fact_transaction.recorder_id NOT NULL',
                 sum(1 for t in fact_transaction if t.get('recorder_id')), len(fact_transaction))
        dq_check(report, 'REQUIRED_FIELD', 'fact_transaction.transaction_date NOT NULL',
    ...
    if len(bridge_transaction_party) > 0:
        dq_check(report, 'REQUIRED_FIELD', 'bridge_transaction_party.party_name_raw NOT NULL',
    ...
    if len(dim_property) > 0:
        dq_check(report, 'UNIQUENESS', 'dim_property.property_key is unique',
        ...
        dq_check(report, 'UNIQUENESS', 'dim_property (tax_assessor_id, valid_from) is unique',
                 len(unique_nk), len(dim_property),
                 message="SCD Type 2 natural key must be unique")
    
    if len(dim_entity) > 0:
        dq_check(report, 'UNIQUENESS', 'dim_entity.entity_key is unique',
                 len(set(e['entity_key'] for e in dim_entity)), len(dim_entity))
    
    if len(fact_transaction) > 0:
        dq_check(report, 'UNIQUENESS', 'fact_transaction.transaction_key is unique',
                 len(set(t['transaction_key'] for t in fact_transaction)), len(fact_transaction))
        dq_check(report, 'UNIQUENESS', 'fact_transaction.recorder_id is unique',
    ...
    if len(fact_transaction) > 0:
        ...
        dq_check(report, 'REFERENTIAL_INTEGRITY', 'fact_transaction.property_key → dim_property',
                 valid_fk, len(txns_with_prop_key),
                 message="All non-null FKs must exist in parent table")
    
    # bridge_transaction_party → fact_transaction
    if len(bridge_transaction_party) > 0:
        valid_bridge = sum(1 for b in bridge_transaction_party if b['transaction_key'] in valid_txn_keys)
        dq_check(report, 'REFERENTIAL_INTEGRITY', 'bridge_transaction_party.transaction_key → fact_transaction',
                 valid_bridge, len(bridge_transaction_party))
    
    # bridge_property_owner → dim_property
    if len(bridge_property_owner) > 0:
        valid_po_prop = sum(1 for b in bridge_property_owner if b['property_key'] in valid_property_keys)
        dq_check(report, 'REFERENTIAL_INTEGRITY', 'bridge_property_owner.property_key → dim_property',
                 valid_po_prop, len(bridge_property_owner))
        
        valid_po_ent = sum(1 for b in bridge_property_owner if b['entity_key'] in valid_entity_keys)
        dq_check(report, 'REFERENTIAL_INTEGRITY', 'bridge_property_owner.entity_key → dim_entity',
                 valid_po_ent, len(bridge_property_owner))
    ...
    if len(dim_property) > 0:
        ...
        dq_check(report, 'CONSISTENCY', 'dim_property: 1 current row per tax_assessor_id',
                 unique_current, current_props if current_props > 0 else 1,
                 message=f"{current_props - unique_current} duplicate current rows" if current_props != unique_current else "No duplicates")
    ...
    if len(fact_transaction) > 0 and len(bridge_transaction_party) > 0:
        ...
        dq_check(report, 'CONSISTENCY', 'bridge grantor count = fact.grantor_count sum',
                 fact_grantor if matches else 0, fact_grantor if fact_grantor > 0 else 1,
    ...
    if len(fact_transaction) > 0 and start_date and end_date:
        ...
        if txn_dates:
            ...
            dq_check(report, 'CONSISTENCY', 'Transactions within date range',
                     in_range, len(txn_dates),
    ...
    if len(fact_transaction) > 0:
        ...
        if len(sales) > 0:
            ...
            dq_check(report, 'BUSINESS_LOGIC', 'Sales have document_amount > 0',
                     sales_with_amount, len(sales))
        
        # Transactions should have parties
        with_parties = sum(1 for t in fact_transaction if t['grantor_count'] > 0 or t['grantee_count'] > 0)
        dq_check(report, 'BUSINESS_LOGIC', 'Transactions have at least one party',
                 with_parties, len(fact_transaction), threshold=95)
    ...
    if len(dim_property) > 0:
        ...
        dq_check(report, 'BUSINESS_LOGIC', 'Properties have valid MF use codes',
                 with_valid_code, len(dim_property), threshold=95)
    ...
    if len(dim_property) > 0:
        ...
        record_stat(report, 'COMPLETENESS', 'dim_property.property_address',
    ...
    if len(fact_transaction) > 0:
        ...
        record_stat(report, 'CARDINALITY', 'Transactions with 2+ grantors',
        ...
        record_stat(report, 'CARDINALITY', 'Transactions with 2+ grantees',
    ...
    if len(fact_transaction) > 0:
        ...
        for cat, count in category_dist.items():
            record_stat(report, 'TRANSACTION_TYPE', f'{cat} transactions',
                        f"{count:,} ({count/len(fact_transaction)*100:.1f}%)")
    
    # ==================== SUMMARY ====================
    logger.info(f"✅ DQ validation complete:")
